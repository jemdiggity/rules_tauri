...
The example-specific Bazel glue builds the frontend assets and release binary from `app/`, then passes those generated outputs through the public `rules_tauri` interface. `rules_tauri` itself still only assembles the unsigned macOS `.app`.
